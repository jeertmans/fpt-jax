...
def generate_dataset(
    *,
    ...
    pbar = tqdm(
        total=total_configs * batch_size, desc="Generating certified cases (tol=1e-14)"
    )
    ...
    pbar.close()
    np.savez_compressed(output_path, allow_pickle=True, **dataset)
    print(f"Saved certified benchmark dataset to {output_path}")


def main():
    parser = argparse.ArgumentParser(
        description="Generate certified benchmark dataset with ultra-low tolerance"
    )
    parser.add_argument(
        "--batch", type=int, default=128, help="Batch size per (scenario, n)"
    )
