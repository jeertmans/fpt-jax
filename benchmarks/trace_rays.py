...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark `trace_rays` solvers against ECOS dataset"
    )
    parser.add_argument(
        "--dataset",
        type=Path,
...
def run_benchmark(
    *,
    ...
    for n in ns:
        ...
        origins = jnp.array(
            dataset_npz[f"{key_prefix}_origins"][:batch], dtype=jnp.float32
        )
        vectors = jnp.array(
            dataset_npz[f"{key_prefix}_vectors"][:batch], dtype=jnp.float32
        )
        itypes = jnp.array(dataset_npz[f"{key_prefix}_itypes"][:batch], dtype=jnp.int32)
        expected = jnp.array(
            dataset_npz[f"{key_prefix}_expected"][:batch], dtype=jnp.float32
        )
...
def main():
    ...
    if not dataset_path.exists():
        raise FileNotFoundError(
            f"Dataset {dataset_path} not found. Please run scripts/generate_benchmark_dataset.py first."
        )
    ...
    for row, (sc_name, p_reflect, num_dims) in enumerate(scenarios):
        ...
        for col, n in enumerate(ns):
            ...
            for cfg in solver_cfgs:
                ...
                xs_clean = [
                    x
                    for x, y in zip(xs[cfg.name], ys[cfg.name])
                    if np.isfinite(x) and np.isfinite(y) and y > 0 and x > 0
                ]
                ys_clean = [
                    y
                    for x, y in zip(xs[cfg.name], ys[cfg.name])
